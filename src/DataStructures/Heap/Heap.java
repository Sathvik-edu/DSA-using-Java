...
public class Heap {
    ...
    //Creates Heap and Initialize to ArrayList
    ...
    // WRITTEN THE INSERT METHOD HERE //
    public void insert(int value) {
        ...
        //----Helper(SinkDown) Method FOR REMOVE----//
    ...
    // WRITTEN THE REMOVE METHOD HERE //
    public Integer remove() {
        // If the heap is empty, there is nothing to remove, so return null.
        if (heap.size() == 0) {
            return null;
        }
        ...
        // Otherwise, the heap has more than one item. The item to be removed
        // from a max heap is always the root, which has the maximum value.
        int maxValue = heap.get(0);

        // Replace the root with the last item in the heap.
        heap.set(0, heap.remove(heap.size() - 1));

        // Restore the max heap property by moving the new root down the heap
        // until it is less than or equal to its children or it becomes a leaf.
        sinkDown(0);
